...
const SNAPSHOT_STORAGE_KEY = 'accounting-github-pages-snapshot-v5';
const PAGE_STORAGE_KEY = 'accounting-github-pages-current-page-v1';
const LEGACY_SNAPSHOT_STORAGE_KEYS = ['accounting-github-pages-snapshot-v1', 'accounting-github-pages-snapshot-v2', 'accounting-github-pages-snapshot-v3', 'accounting-github-pages-snapshot-v4'];
...
function migrateSeedJournals(journals) {
  if (!Array.isArray(journals)) return [];

  const seedIds = ['seed-journal-1', 'seed-journal-2', 'seed-journal-3', 'seed-journal-4'];
  const journalIds = journals.map(journal => String(journal?.id || ''));
  const hasExactLegacySeedSet = journals.length === seedIds.length
    && seedIds.every((seedId, index) => journalIds[index] === seedId);

  if (!hasExactLegacySeedSet) return journals;
  return journals.slice(0, 2);
}

function normalizeSnapshot(snapshot) {
  return {
    accounts: normalizeCollection(snapshot?.accounts),
    journals: migrateSeedJournals(normalizeCollection(snapshot?.journals)),
...
function navigate(page) {
  ...
  if (page !== 'trial') hideTrialAddForm();
...
function resetTrialAddForm() {
  const codeInput = document.getElementById('trialAddCode');
  const drInput = document.getElementById('trialAddDr');
  const crInput = document.getElementById('trialAddCr');
  if (codeInput) codeInput.value = '';
  if (drInput) drInput.value = '';
  if (crInput) crInput.value = '';
}

function hideTrialAddForm() {
  const panel = document.getElementById('trialAddPanel');
  if (panel) panel.style.display = 'none';
  resetTrialAddForm();
}

function showTrialAddForm() {
  const panel = document.getElementById('trialAddPanel');
  if (!panel) return;
  panel.style.display = 'block';
  resetTrialAddForm();
  document.getElementById('trialAddCode')?.focus();
}

function syncTrialAddCodeSelection() {
  const code = document.getElementById('trialAddCode')?.value.trim() || '';
  const account = getAccountByCode(code);
  if (!account || !account.is_posting) {
    document.getElementById('trialAddDr').value = '';
    document.getElementById('trialAddCr').value = '';
    return;
  }

  const current = getOpeningBalances()[code] || { dr: 0, cr: 0 };
  document.getElementById('trialAddDr').value = current.dr ? current.dr : '';
  document.getElementById('trialAddCr').value = current.cr ? current.cr : '';
}

async function saveTrialOpeningEntry() {
  const code = document.getElementById('trialAddCode')?.value.trim() || '';
  const dr = parseFloat(document.getElementById('trialAddDr')?.value) || 0;
  const cr = parseFloat(document.getElementById('trialAddCr')?.value) || 0;

  if (!code) {
    showToast('Select an account code first', 'error');
    return;
  }

  const account = getAccountByCode(code);
  if (!account) {
    showToast('This account code does not exist', 'error');
    return;
  }

  if (!account.is_posting) {
    showToast('Beginning of year values can only be added to posting accounts', 'error');
    return;
  }
  ...
  if (dr === 0 && cr === 0) {
    showToast('Enter a debit or credit beginning balance', 'error');
    return;
  }

  try {
    await persistOpeningBalanceRecord(code, dr, cr, true);
    hideTrialAddForm();
    renderTrialBalance();
  } catch (error) {
    showToast(error.message, 'error');
...
function updatePostingAccountsList() {
  ...
  const options = posting.map(a => `<option value="${a.code}" label="${a.name}">`).join('');

  ['postingAccountsList', 'trialPostingAccountsList'].forEach(id => {
...
function renderTrialBalance() {
  ...
  const status = document.getElementById('trialOpeningStatus');
  body.innerHTML = '';
  foot.innerHTML = '';
  if (status) {
    status.textContent = '';
    status.className = 'trial-opening-status';
  ...
  if (status) {
    const openingDiff = totOpenDr - totOpenCr;
    const absoluteDiff = Math.abs(openingDiff);

    if (absoluteDiff >= 0.01) {
      const sideText = openingDiff > 0 ? 'Debit exceeds Credit' : 'Credit exceeds Debit';
      status.textContent = `Beginning of year balances are not balanced. ${sideText} by ${fmtNum(absoluteDiff)}.`;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('addTrialOpeningBtn')?.addEventListener('click', showTrialAddForm);
  document.getElementById('saveTrialOpeningBtn')?.addEventListener('click', saveTrialOpeningEntry);
  document.getElementById('cancelTrialOpeningBtn')?.addEventListener('click', hideTrialAddForm);
  document.getElementById('trialAddCode')?.addEventListener('change', syncTrialAddCodeSelection);
  document.getElementById('trialAddCode')?.addEventListener('input', syncTrialAddCodeSelection);
